...
Why: README/AGENTS/ROADMAP describe Next 14 + MongoDB + Mongoose + NextAuth; TASK-DD-MON-001 cites deleted `models/*` paths; `next.config.mjs:35` still externalizes mongoose.
Files: `README.md`, `AGENTS.md`, `docs/ROADMAP.md`, `docs/tasks/monetization/TASK-DD-MON-001-stripe-pro-checkout/spec.md` (source paths note only; the rewrite itself is DD-C09), `next.config.mjs`.
