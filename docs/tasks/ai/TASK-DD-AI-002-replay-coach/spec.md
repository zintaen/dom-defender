...
owner: Stephen Cheng
created: 2026-06-24
related_frs: [TASK-DD-AI-001, TASK-DD-EDU-001]
depends_on: [TASK-DD-AI-001]
blocks: []

# Source contracts
source_decisions:
  - lib/game/replay.ts, models/Replay.ts (the event log + snapshots to analyze)
  - lib/game/director.types.ts (reuse the AI seam from TASK-DD-AI-001)
...
effort_hours: 8
risk_if_skipped: "The game shows a score but never tells the player why they lost or how to improve. A coach that points at the real mistake in their run is the difference between a one-time curiosity and a skill they want to grow, and it is the teaching layer the teams mode (TASK-DD-EDU-001) builds on."
---
...
for a given replay, with no network call. The function reuses the AI seam from TASK-DD-AI-001, so a
...
Depends on TASK-DD-AI-001 for the shared decision/seam types. This is the second concrete "AI"
the player feels, and it is the bridge to TASK-DD-EDU-001: in teams mode the same coach output
